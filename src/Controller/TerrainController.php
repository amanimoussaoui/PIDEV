   ...
   public function new(Request $request, EntityManagerInterface $entityManager): Response
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        // Affecter la latitude et la longitude à l'entité
...
public function mesTerrains(TerrainRepository $terrainRepository): Response
{
   ...
   #[Route('/showterrainback', name: 'app_showterrainback')]
   public function showterrainback(TerrainRepository $terrainRepository): Response
   {
       $user = $this->getUser(); // Récupérer l'utilisateur connecté
       if (!$user) {
           throw $this->createAccessDeniedException("Vous devez être connecté pour voir vos terrains.");
       ...
       return $this->render('terrain/showterrainback.html.twig', [
           'tab_terrain' => $terrains,
       ]);
   }
...
public function updateformterrain(ManagerRegistry $m, Request $req, $id, TerrainRepository $rep, #[Autowire('%uploads_dir%')] string $uploadsDir): Response
{
 ...
 #[Route('/terrain/{id}', name: 'app_affichage_terrain', requirements: ['id' => '\d+'])]
public function afficherTerrain(int $id, TerrainRepository $terrainRepository, CandidatureRepository $candidatureRepository): Response
{
    ...
    $googleMapsApiKey = $_ENV['GOOGLE_MAPS_API_KEY'];  // Assurez-vous que la clé API est bien définie dans le .env

    return $this->render('terrain/details.html.twig', [
        'terrain' => $terrain,
        'candidatures' => $candidatures,
        'google_maps_api_key' => $googleMapsApiKey, // Passer la clé API à la vue
    ]);
}
...
#[Route('/terrain/recherche', name: 'terrain_recherche', methods: ['GET'])]
public function rechercher(TerrainRepository $terrainRepository, Request $request): Response
{
    $localisation = $request->query->get('localisation', '');

    $terrains = [];
    if (!empty($localisation)) {
        $terrains = $terrainRepository->findByLocalisation($localisation);
    }
...
//GOOGLE MAPS 
    ...
    #[Route('/choisirLocalisation', name: 'choisirLocalisation')]
public function choisirLocalisation(Request $request): Response
{
    $googleMapsApiKey = $this->getParameter('google_maps_api_key');
    return $this->render('terrain/choisirLocalisation.html.twig', [
...
#[Route('/favori/ajouter/{id}', name: 'ajouter_favori', methods: ['POST'])]
public function ajouterFavori(Terrain $terrain, SessionInterface $session)
{
    // Récupérer les favoris de la session
    $favoris = $session->get('favoris', []);

    // Ajouter l'ID du terrain aux favoris (si ce n'est pas déjà le cas)
    if (!in_array($terrain->getId(), $favoris)) {
        $favoris[] = $terrain->getId();
...
#[Route('/favori/supprimer/{id}', name: 'supprimer_favori', methods: ['POST'])]
public function supprimerFavori(Terrain $terrain, SessionInterface $session)
{
    $favoris = $session->get('favoris', []);

    if (in_array($terrain->getId(), $favoris)) {
        $favoris = array_diff($favoris, [$terrain->getId()]); // Supprimer l’ID du tableau
        $session->set('favoris', $favoris);
    }
...
public function toggleFavori(Terrain $terrain, SessionInterface $session)
{
    // Récupérer les favoris de la session
    $favoris = $session->get('favoris', []);

    // Vérifier si le terrain est déjà favori
    if (in_array($terrain->getId(), $favoris)) {
        // Supprimer le favori
        $favoris = array_diff($favoris, [$terrain->getId()]);
    } else {
        // Ajouter le favori
        $favoris[] = $terrain->getId();
    }
    ...
    return $this->json(['success' => true, 'isFavori' => in_array($terrain->getId(), $favoris)]);
}

#[Route('/favoris', name: 'favoris')]
public function afficherFavoris(SessionInterface $session, EntityManagerInterface $entityManager)
{
    // Récupérer les IDs des terrains favoris de la session
    $favoris = $session->get('favoris', []);

    // Récupérer les terrains favoris
    $terrainsFavoris = !empty($favoris) ? 
        $entityManager->getRepository(Terrain::class)->findBy(['id' => $favoris]) : 
        [];

    return $this->render('terrain/favoris.html.twig', [
        'terrainsFavoris' => $terrainsFavoris,
    ]);
}

/**
 * @Route("/update-favori/{terrainId}", name="update_favori", methods={"POST"})
 */
public function updateFavori(Request $request, $terrainId)
{
    $session = $request->getSession();
    $favoris = $session->get('favoris', []);

    if (in_array($terrainId, $favoris)) {
        // Supprimer des favoris
        $favoris = array_filter($favoris, fn($id) => $id !== $terrainId);
    } else {
        // Ajouter aux favoris
        $favoris[] = $terrainId;
    }
...
#[Route('/terrain/statistics', name: 'app_terrain_statistics')]
public function statistics(TerrainRepository $terrainRepository): Response
{
    // Récupérer les terrains les plus demandés
    $topTerrains = $terrainRepository->findTopTerrainsWithMostCandidatures(5);

    // Extraire les noms des terrains et le nombre de candidatures
    $labels = [];
    $data = [];
    
    foreach ($topTerrains as $terrain) {
        $labels[] = $terrain[0]->getId(); // Récupérer le nom du terrain
        $data[] = $terrain['candidatureCount']; // Nombre de candidatures
    }

    return $this->render('terrain/statistics.html.twig', [
        'labels' => $labels,
        'data' => $data,
    ]);
}
