...
public class MainmenuScene {
    ...
    private void redirectToMateriels() {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/MaterielSelectionScene.fxml"));
            Stage stage = (Stage) btnProducts.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.show();
        } catch (IOException e) {
            System.err.println("Erreur de chargement de MaterielSelectionScene: " + e.getMessage());
            e.printStackTrace();
        }
    }
